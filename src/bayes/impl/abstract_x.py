...
# The backward star is a list containing each node's predecessor. See e.g. bst_simple, bst_chain
# and others.
...
# fpt[0][0] = p(X0 = 0)
#
# Again, look at fpt_simple, fpt_chain and others
#

import numpy as np

from by.util.binaries import binary_cube, fpt_equals, rev_int, log2
...
class BayesianNetwork(object):
    def __init__(self, name, bst, fpt):
        ...
        self.fpt = fpt2np(fpt)  # self.forward_probability_table()
        self.name = name
    ...
    def joint_probability_internal(self, values):
        ...
        for k in range(len(values)):
            v = (values[i] for i in self.bst[k])  # values of predecessors of k
            fp = self.forward_probability_table()[k][tuple(v)]
    ...
    def backward_probability_table_(self, full):
        """
        :param full: if full is False, the successors are the ones given bx forward star (fst)
        if full is True, for node i all nodes from i+1 through n-1 are taken as successors
        :return: the backward probability table
        ...
        for i in range(n):
            ...
            if slen == 0:  # compute p(xi = 0) (unconditional probability)
                bpt[i] = np.zeros((1,))
                bpt[i][0] = self.conditional_probability(pattern, successors[i])
            else:  # enumerate values v of successors of i
                bpt[i] = np.zeros((2,) * slen)
                for v in binary_cube([None] * slen):
                    for t in range(slen):
                        pattern[successors[i][t]] = v[t]
                    bpt[i][tuple(v)](self.conditional_probability(pattern, successors[i]))
        return bpt
